...
| `global-components-msal-redirect.html` | `packages/cps-global-auth/`                 | `{{AD_CLIENT_ID}}` and `{{AD_TENANT_AUTHORITY}}` substituted from `config.json` at deploy time (loader script reads them from `data-*` attributes when injecting `msal-redirect.js`); referenced from `msalRedirectUrl` in configs |
| `msal-redirect.js` (+ `.map`)          | `packages/cps-global-auth/dist/`            | none — byte-identical IIFE bundle; loaded by `global-components-msal-redirect.html` to run `handleRedirectPromise()` on the termination page |
