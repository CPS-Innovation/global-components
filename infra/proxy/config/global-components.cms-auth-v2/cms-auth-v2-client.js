 ...
 *
 * To call an endpoint instead of (as well as) logging, assign a handler:
 *   window.__ccContactLogger.onChange = function (kind, rec) { ... };
 * where kind is "editing"|"closed" and rec is
 *   { key, personId, recorderId, name, role }.
...
(function () {
  ...
  function readOpenContact(win) {
    ...
    var rec = {};
    rec.key = personId + "/" + (recorderId || "-") + "/" + role;
  ...
  // Report a transition. Logs, then hands off to an optional consumer hook.
  // The hook is fully isolated: anything it throws is swallowed here.
  function emit(kind, rec) {
  ...
  function tick() {
    ...
    lastKey = key;
    if (prev) {
      emit("closed", prev); // includes the A -> B switch case
    }
    if (rec) {
      emit("editing", rec);
    }
