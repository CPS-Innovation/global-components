...
2. Validate Bearer tokens issued by a _different_ App Registration (App B) at the same tenant
...
The `oid` claim (user's Azure AD Object ID) is identical in tokens from _any_ App Registration within the same tenant. This means:
