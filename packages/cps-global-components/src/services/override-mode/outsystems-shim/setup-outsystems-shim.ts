...
const hideOSHeader = () => {
  log("[Header Hider Extension] HIDING the OS header");
  ...
  headers.forEach((header: HTMLElement) => {
    if (header && header.style.display !== "none") {
      ...
      log("[Header Hider Extension] HIDDEN the OS header");
    }
  });
  ...
  if (blueLine && blueLine.style.display !== "none") {
    ...
    log("[Header Hider Extension] HIDING the OS blue bar");
  }
  ...
  if (floatingNav && floatingNav.style.display !== "none") {
    ...
    log("[Header Hider Extension] HIDING the floating nav");
  }
  log("[Header Hider Extension] HIDING the OS header - done");
};

const ensureGlobalHeader = () => {
  log("[Header Hider Extension] ENSURING global nav");
  if (document.querySelector("cps-global-header")) {
    log("[Header Hider Extension] ENSURING global nav - already exists");
    return;
  }
  ...
  if (!mainDiv) {
    log("[Header Hider Extension] ENSURING global nav - no main div");
    return;
  }
  ...
  log('[Header Hider Extension] cps-global-header component added to div[role="main"]');
  ...
  while (ancestor) {
    ...
    if (computedStyle.position === "sticky") {
      ...
      log("[Header Hider Extension] Removed position: sticky from ancestor element", ancestor);
    }
  ...
  mutations.forEach(function (mutation) {
    ...
    mutation.removedNodes.forEach(node => {
      if (node.nodeType === 1 && node.nodeName === "CPS-GLOBAL-HEADER") {
        log("[Header Hider Extension] cps-global-header component was REMOVED from DOM");
      }
    });
    ...
    mutation.addedNodes.forEach(node => {
      if (node.nodeType === 1 && node.nodeName === "CPS-GLOBAL-HEADER") {
        log("[Header Hider Extension] cps-global-header component was ADDED back to DOM");
      }
    });
...
export const setupOutSystemsShim = () => {
