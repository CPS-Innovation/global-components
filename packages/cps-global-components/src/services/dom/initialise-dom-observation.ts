...
type Props = { window: Window; register: Register };

const cache: { observer?: ReturnType<typeof setupMutationObserver>; contextIndex?: number } = {};

export const initialiseDomObservation = ({ window, register }: Props) => {
  const resetDomObservation = ({ context: { domTags, contextIndex } }: { context: FoundContext }) => {
    ...
    cache.observer = setupMutationObserver(window.document.body, domTags, tags => register({ tags }));
  };
