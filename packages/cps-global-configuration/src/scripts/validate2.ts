...
import { validateConfig2 } from "../validator";
...
function validateFile(filePath: string): boolean {
  try {
    ...
    const result = validateConfig2(jsonData, filename);
...
function main() {
  ...
  if (args.length === 0) {
    console.error("Usage: node validate2.js <folder-path>");
    console.error("Example: node validate2.js ../../configuration2");
    process.exit(1);
  }
