...
- Upload this file to the root of the `Data` tab -> `Resources` folder of the test harness app. Set the `Deploy Action` to `Deploy to Target Directory`.

- Upload the files in `assets/fonts/` to the same root `Resources` folder. For each file set the `Deploy Action` to `Deploy to Target Directory`.

- Set the theme background to white.

- Switch off the fade transition.
